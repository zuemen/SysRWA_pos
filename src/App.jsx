      ...
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
          <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-slate-900 border border-purple-500/30 p-10 rounded-[40px] max-w-xl w-full shadow-2xl overflow-hidden relative">
            <div className="flex justify-between items-center mb-8">
              <h4 className="text-2xl font-black text-white flex items-center gap-3 uppercase italic tracking-tighter"><Fingerprint className="text-purple-400" /> 設備數位身分證 (SSI-VC)</h4>
            ...
            {showRaw ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-slate-950 p-6 rounded-2xl border border-slate-800 font-mono text-[9px] text-purple-300 h-64 overflow-y-auto custom-scrollbar italic leading-relaxed">
                <pre>{JSON.stringify({
    ...
    <div className="flex items-center gap-4 mb-4">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-[9px] font-black text-green-400 uppercase tracking-tighter">{trend}</span>
          <span className={`text-[8px] px-2 py-0.5 rounded-md font-black tracking-tighter uppercase ${clickable ? 'bg-cyan-500 text-slate-950 shadow-sm' : 'bg-slate-800 text-slate-500'}`}>{status}</span>
        </div>
      </div>
    </div>
